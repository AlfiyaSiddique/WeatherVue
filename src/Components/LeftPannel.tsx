...
const LeftPannel: React.FC<{weather: WeatherDataInterface, getData: getDataInterface}> = ({weather, getData}) => {
    ...
    <div>
      ...
      <div className="relative w-[80%] mx-auto my-8">
          ...
          <input
            ...
            onKeyUp={(e)=>{
                if(e.code === "Enter") {getData(url); setNewCity(city)}

            }}
            onChange={(e)=>setCity(e.target.value)}
            value={city}
          />
            <button type="submit" onClick={()=>{getData(url), setNewCity(city)}} className="text-white absolute end-0 bottom-0 bg-gray-700 hover:bg-gray-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium  text-sm px-4 py-[13px] rounded-r-md dark:bg-gray-600 dark:hover:bg-gray-700 dark:focus:ring-gray-800">Search</button>
        </div>
        <div className="flex flex-wrap justify-around my-4 w-full text-center">
        {Object.keys(weather).slice(0,5).map((val , index)=>{
            return <WeatherCard key={index} weatherKey={val} value={weather[val]}/>
        })}
        </div>
